package me.jraynor.client.screens;
...
import me.jraynor.common.util.CommonUtils;
...
public class UtilityScreen extends ContainerScreen<UtilityContainer> {
    ...
    private void addOffsets() {
        ...
        addButton(new TransferStateButton((centerX + centerOffset) + 4, (centerY + centerOffset) + 4, onTransferUpdate, (button, stack, mouseX, mouseY) -> {
            ...
            func_243308_b(stack, Lists.newArrayList(new StringTextComponent(CommonUtils.capitalize(btn.getState().name().toLowerCase()) + " mode")), mouseX, mouseY);
        }));
    }
    ...
    private String getFace(Direction direction) {
        ...
        faceToDir.forEach((facing, direction1) -> {
            if (direction1 == direction)
                name.set(CommonUtils.capitalize(facing.name().toLowerCase()));
        });
        return name.get();
    }
        ...
        if (connections != null)
            connections.forEach(connectionRef -> {
                ...
                if (button.getTransferType() == connection.getMode()) {
                    ...
                    list.add(new StringTextComponent(" - " + TextFormatting.DARK_GRAY + "Operation: " + TextFormatting.DARK_PURPLE + connection.getIo().name().toLowerCase()));
